...
// Vendored (checked into git) rather than pulled from node_modules at build
// time, since this static site is served directly from the repo root with
// no bundler -- see vendor/purchases-js/README.md.
await mkdir(join(output, 'vendor', 'purchases-js'), { recursive: true });
await cp(
  join(root, 'vendor', 'purchases-js', 'Purchases.es.js'),
  join(output, 'vendor', 'purchases-js', 'Purchases.es.js'),
);
